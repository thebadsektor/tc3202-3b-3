...
#this is for predicting the values of user 
...
#matching the values of users to the values of candidates
...
#         # Step 1: Predict values from answers (returns a text)
#         raw_values_text = predict_values_based_on_answers(answers)

#         # Step 2: Convert text response to a list
#         predicted_values = [
#             line.replace("*", "").strip()
...
#             print(f"  - {candidate['candidate_name']} ({candidate['party']}) -> Match Score: {(1 - candidate['distance']):.2f}")
