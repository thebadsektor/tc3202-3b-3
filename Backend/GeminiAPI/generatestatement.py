...
# Load the CSV using a forgiving encoding
try:
    ...
    raise UnicodeDecodeError("Failed to decode 'politician_articles.csv'. Try re-saving as UTF-8.")
...
# # Dynamically build correct absolute path to CSV file
# base_dir = os.path.dirname(os.path.abspath(__file__))  # /Backend/GeminiAPI
# csv_path = os.path.abspath(os.path.join(base_dir, "..", "cleaned_utf8_file.csv"))
...
# except UnicodeDecodeError:
#     raise UnicodeDecodeError("Failed to decode 'cleaned_utf8_file.csv'. Try re-saving as UTF-8.")
