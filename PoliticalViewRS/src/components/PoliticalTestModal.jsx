    ...
    <div className='fixed inset-0 z-50 backdrop-blur-sm bg-black/30 flex items-center justify-center'>
      <div className='bg-white text-black rounded-xl shadow-lg p-8 max-w-md w-full relative flex flex-col items-center text-center'>
        ...
        <select
          value={language}
          onChange={(e) => setLanguage(e.target.value)}
          className='mb-4 w-full px-4 py-2 border border-black rounded-md'
        >
          <option value=''>-- Select Language --</option>
          <option value='tagalog'>Tagalog</option>
          <option value='english'>English</option>
        ...
        <button
          onClick={handleStart}
          className='bg-black hover:bg-[#303030] text-white py-3 px-6 rounded-md hover:cursor-pointer font-semibold w-full'
        >
          Start the test
        </button>
      </div>
